(function () {
    ...
    // Изменил версию на 99, чтобы Lampa точно сбросила кэш старого сломанного плагина
    ...
    function openActorPage(a) {
        ...
        if (id && /^\d+$/.test(String(id))) {
            try {
                Lampa.Activity.push({ url: 'person/' + id, title: a.name || '', component: 'actor', id: id, source: 'tmdb', page: 1 });
                return;
            } catch (e) {}
            try {
                Lampa.Activity.push({ url: 'person/' + id, title: a.name || '', component: 'person', id: id, source: 'tmdb', page: 1 });
                return;
            } catch (e2) {}
        }
        Lampa.Activity.push({
    ...
    // Универсальная функция плавного скролла для пультов ТВ
    function addScrollFocus(el) {
        el.on('hover:focus', function () {
            ...
            try {
                ...
                try { node.scrollIntoView(); } catch (er) {}
            }
        });
        return el;
    }
    ...
    var CardCache = {
        map: {}, loading: {},
        key: function (m) { return m ? String(m.id || m.tmdb_id || m.imdb_id || '') : ''; },
        put: function (m) {
            var k = this.key(m);
            if (!k || !m) return;
            var prev = this.map[k] || {};
            var next = Object.assign({}, prev, m);
            next.genres = (m.genres && m.genres.length ? m.genres : null) || (prev.genres && prev.genres.length ? prev.genres : null) || m.genres || prev.genres;
            next.genre_ids = (m.genre_ids && m.genre_ids.length ? m.genre_ids : null) || (prev.genre_ids && prev.genre_ids.length ? prev.genre_ids : null) || m.genre_ids || prev.genre_ids;
            var prevCast = (prev.credits && prev.credits.cast && prev.credits.cast.length && prev.credits) || (prev.persons && prev.persons.cast && prev.persons.cast.length && prev.persons) || null;
            var newCast = (m.credits && m.credits.cast && m.credits.cast.length && m.credits) || (m.persons && m.persons.cast && m.persons.cast.length && m.persons) || null;
            if (newCast && newCast.cast) { next.credits = { cast: newCast.cast }; if (m.persons) next.persons = m.persons; }
            else if (prevCast && prevCast.cast) { next.credits = { cast: prevCast.cast }; if (prev.persons) next.persons = prev.persons; }
        ...
        get: function (m) {
            if (!m) return null;
            var k = this.key(m);
            return k && this.map[k] ? Object.assign({}, m, this.map[k]) : m;
        },
        hasRich: function (m) {
            if (!m) return false;
            var c = this.get(m);
            var hasGenre = (c.genres && c.genres.length) || (c.genre_ids && c.genre_ids.length);
            var hasCast = (c.credits && c.credits.cast && c.credits.cast.length) || (c.cast && c.cast.length) || (c.persons && c.persons.cast && c.persons.cast.length);
    ...
    var Media = {
        ...
        getId: function (movie) {
            ...
            var id = movie.id || movie.tmdb_id || movie.kinopoisk_id || movie.imdb_id || movie.original_title || movie.original_name || movie.title || movie.name || 'unknown';
            return String(id) + ':' + String(movie.season_number || movie.season || 0) + ':' + String(movie.episode_number || movie.episode || 0);
        },
        getBaseId: function (movie) {
            if (!movie) return 'unknown';
            return String(movie.id || movie.tmdb_id || movie.kinopoisk_id || movie.imdb_id || movie.original_title || movie.original_name || movie.title || movie.name || 'unknown');
        },
        isEpisode: function (movie) {
            if (!movie) return false;
            return !!(movie.episode || movie.episode_number || ((movie.season_number || movie.season) && (movie.episode_number || movie.episode)));
        },
        ...
        actorsOf: function (movie) {
            ...
            return list.filter(function (p) { return p && (p.id || p.name); }).slice(0, 15).map(function (p) {
                return { id: p.id || p.name, name: p.name || '', profile_path: p.profile_path || p.img || p.photo || '' };
            });
        },
        posterOf: function (movie) { return movie ? (movie.poster_path || movie.img || movie.poster || '') : ''; },
        titleOf: function (movie) { return movie ? (movie.title || movie.name || movie.original_title || movie.original_name || '') : ''; },
        normalize: function (data) {
            if (!data) return null;
            var movie = data.card || data.movie || null;
            if (!movie || typeof movie !== 'object') {
                if (data.id || data.title || data.name || data.original_title) movie = data; else return null;
            }
        ...
        labelOf: function (movie) {
            return movie ? (movie.id || movie.tmdb_id || movie.title || movie.name || movie.original_title || movie.original_name || '?') : '?';
        }
    };
    ...
    var MetaStore = {
        ...
        save: function (movie) {
            ...
            if (!id) return;
            try {
                var meta = { id: id, title: Media.titleOf(movie), release_date: movie.release_date || '', first_air_date: movie.first_air_date || '', poster_path: Media.posterOf(movie), genres: Media.genresOf(movie), genre_ids: movie.genre_ids || [], actors: Media.actorsOf(movie), year: Media.yearOf(movie) };
        ...
        load: function (idOrMovie) {
            ...
            if (!id) return null;
            try { var m = Lampa.Storage.get(this.prefix + id); return m && typeof m === 'object' ? m : null; }
            catch (e) { return null; }
        },
        applyToMovie: function (movie) {
            ...
            if (stored.actors && stored.actors.length) { if (!(movie.credits && movie.credits.cast && movie.credits.cast.length)) movie.credits = { cast: stored.actors }; }
    ...
    var DomMeta = {
        scrape: function () {
            ...
            try {
                ...
                if (y) { var ym = (y.textContent || '').match(/(19|20)\d{2}/); if (ym) result.year = parseInt(ym[0], 10); }
                if (!result.year) {
                    var head = document.querySelector('.full-start-new__head, .full-start__head');
                    if (head) { var hm = (head.textContent || '').match(/(19|20)\d{2}/); if (hm) result.year = parseInt(hm[0], 10); }
                }
                ...
                if (posterEl) { result.poster = posterEl.getAttribute('src') || posterEl.getAttribute('data-src') || (posterEl.style && posterEl.style.backgroundImage ? String(posterEl.style.backgroundImage).replace(/url\(["']?/, '').replace(/["']?\)/, '') : ''); }
                ...
                if (details) {
                    ...
                    parts.forEach(function (p) { if (isGarbageGenre(p)) return; result.genres.push({ name: p }); });
                }
                var actorNodes = document.querySelectorAll('.full-person__name, .person__name, .full-persons .card__title, .persons .card__title, .full-person .card__title');
                actorNodes.forEach(function (el, i) {
                    if (i > 14) return;
                    var name = (el.textContent || '').trim();
                    if (!name) return;
                    var img = '', personId = null, root = el.closest('.full-person, .person, .card, .full-persons__item');
                    if (root) {
        ...
        mergeInto: function (movie) {
            ...
            if (d.actors.length && !(movie.credits && movie.credits.cast && movie.credits.cast.length)) movie.credits = { cast: d.actors };
    ...
    Media.metaFrom = function (movie) {
        ...
        return { isEpisode: Media.isEpisode(movie), year: Media.yearOf(movie), genres: Media.genresOf(movie), actors: Media.actorsOf(movie), poster: Media.posterOf(movie), title: Media.titleOf(movie), baseId: Media.getBaseId(movie), watchId: Media.getId(movie) };
    };
    ...
    var MetaLoader = {
        ...
        lang: function () {
            try { return Lampa.Storage.field('tmdb_lang') || Lampa.Storage.field('language') || 'uk-UA'; }
        ...
        enrich: function (movie, done) {
            ...
            if (CardCache.hasRich(movie) || (Media.genresOf(movie).length && Media.actorsOf(movie).length)) { Media.remember(movie); if (done) done(CardCache.get(movie) || movie); return; }
            var key = String(id);
            ...
            function finish(data) {
                ...
                if (Tracker.currentMovie && CardCache.key(Tracker.currentMovie) === key) Tracker.currentMovie = CardCache.get(merged) || merged;
                try {
                    var watchedId = Media.getId(merged);
                    var last = StatsDB.getLast(watchedId);
                    if (last > 0) { var meta = Media.metaFrom(merged); StatsDB.enrichMetaOnly(Math.min(last, 600), meta); }
                } catch (e) {}
            ...
            try {
                if (Lampa.TMDB && typeof Lampa.TMDB.api === 'function') {
                    ...
                    if (Lampa.Reguest) {
                        ...
                        req.timeout(10000);
                        req.silent(url, function (data) { finish(data); }, function () { self.tryNetwork(url, movie, type, id, finish); });
                        return;
                    }
                }
            } catch (e) {}
        ...
        tryNetwork: function (url, movie, type, id, finish) {
            ...
            try {
                ...
                if (url && Lampa.Network && typeof Lampa.Network.silent === 'function') {
                    Lampa.Network.silent(url, function (data) { finish(data); }, function () { self.tryApiFull(movie, type, id, finish); });
                    return;
                }
            } catch (e) {}
            try {
                if (url && typeof fetch === 'function') {
                    fetch(url).then(function (r) { return r.json(); }).then(function (data) { finish(data); }).catch(function () { self.tryApiFull(movie, type, id, finish); });
                    return;
                }
            } catch (e) {}
        ...
        tryApiFull: function (movie, type, id, finish) {
            try {
                if (Lampa.Api && typeof Lampa.Api.full === 'function') {
                    Lampa.Api.full({ id: id, method: type === 'tv' ? 'tv' : 'movie', card: movie, source: 'tmdb' }, function (data) {
                        if (data && data.movie) {
                            var m = data.movie;
                            if (data.persons && data.persons.cast) { m = Object.assign({}, m, { credits: { cast: data.persons.cast }, persons: data.persons }); }
                            finish(m);
                        } else finish(null);
    ...
    var StatsDB = {
        ...
        init: function () {
            ...
            Object.keys(this.data.actors).forEach(function (k) { if (!StatsDB.data.actors[k].works) StatsDB.data.actors[k].works = {}; });
            this.cleanupGenres();
            this.normalize();
        },
        ...
        save: function () { this.normalize(); try { Lampa.Storage.set(CONFIG.stats_storage, this.data); } catch (e) {} },
        reset: function () { this.data = JSON.parse(JSON.stringify(DEFAULT_STATS)); this.save(); },
        getLast: function (id) { var v = this.data.last_recorded[id]; return Number.isFinite(v) ? v : 0; },
        setLast: function (id, t) { this.data.last_recorded[id] = Math.max(0, Math.floor(t)); },
        ensureActor: function (a) {
            if (!a || !(a.id || a.name)) return null;
            var key = String(a.id || a.name);
            if (!this.data.actors[key]) { this.data.actors[key] = { id: a.id || a.name, name: a.name || '', profile_path: a.profile_path || '', seconds: 0, count: 0, works: {} }; }
        ...
        touchActorWork: function (a, meta, sec, completed) {
            ...
            if (!row.works[wid]) { row.works[wid] = { title: meta.title || '', poster: meta.poster || '', isEpisode: !!meta.isEpisode, seconds: 0, completed: false, date: Date.now() }; }
            var w = row.works[wid];
            ...
            w.date = Date.now();
            if (completed && !w.completed) w.completed = true;
        ...
        addProgress: function (id, time, duration, deltaSec, meta) {
            ...
            if (safe > 0) { this.data.seconds_watched += Math.floor(safe); this.enrich(Math.floor(safe), meta); }
            this.setLast(id, time); this.save(); return Math.floor(safe);
        },
        ...
        markCompleted: function (id, meta, movie) {
            ...
            var entry = { date: Date.now(), isEpisode: !!(meta && meta.isEpisode), title: (meta && meta.title) || Media.titleOf(movie) || '', poster: (meta && meta.poster) || Media.posterOf(movie) || '', tmdb_id: movie && (movie.id || movie.tmdb_id) || '' };
            if (!entry.poster && movie) { var st = MetaStore.load(movie); if (st && st.poster_path) entry.poster = st.poster_path; }
            if (!entry.poster && Media.lastCard) { entry.poster = Media.posterOf(Media.lastCard); if (!entry.title) entry.title = Media.titleOf(Media.lastCard); }
            this.data.completed[id] = entry;
            if (entry.isEpisode) this.data.episodes_watched++; else this.data.movies_watched++;
            if (meta && meta.genres) {
                meta.genres.forEach(function (g) {
                    var name = typeof g === 'string' ? g : (g && g.name);
                    if (!name || isGarbageGenre(name)) return;
            ...
            if (meta && meta.actors) { meta.actors.slice(0, 15).forEach(function (a) { StatsDB.touchActorWork(a, meta, 0, true); }); }
            if (meta && meta.year) {
                var y = String(meta.year);
                if (!this.data.years[y]) this.data.years[y] = { seconds: 0, count: 0 };
        ...
        topActors: function (limit) {
            return Object.keys(this.data.actors).map(function (k) {
                ...
                a._key = k; return a;
            }).sort(function (a, b) { return (b.seconds || 0) - (a.seconds || 0); }).slice(0, limit || CONFIG.max_actors_show);
        },
        ...
        formatTime: function (sec) {
            ...
            var totalMin = Math.floor(sec / 60), days = Math.floor(totalMin / 1440), hours = Math.floor((totalMin % 1440) / 60), minutes = totalMin % 60;
        ...
        genreList: function () {
            return Object.keys(this.data.genres).filter(function (name) { return !isGarbageGenre(name); }).map(function (name) { return { name: name, seconds: StatsDB.data.genres[name].seconds || 0 }; }).sort(function (a, b) { return b.seconds - a.seconds; });
        },
        yearBuckets: function () {
            ...
            return Object.keys(buckets).map(function (k) { return { label: k, seconds: buckets[k] }; }).sort(function (a, b) { return a.label.localeCompare(b.label); });
        },
        bestMonth: function () {
            var best = -1, sec = -1;
            Object.keys(this.data.by_month).forEach(function (m) { var s = StatsDB.data.by_month[m] || 0; if (s > sec) { sec = s; best = parseInt(m, 10); } });
            return best < 0 ? null : { label: MONTHS[best] || String(best), text: StatsDB.formatHoursOrMin(sec) };
        },
        bestWeekday: function () {
            var best = -1, sec = -1;
            Object.keys(this.data.by_weekday).forEach(function (d) { var s = StatsDB.data.by_weekday[d] || 0; if (s > sec) { sec = s; best = parseInt(d, 10); } });
            return best < 0 ? null : WEEKDAYS[best] || '';
        },
        bestHour: function () {
            var best = -1, sec = -1;
            Object.keys(this.data.by_hour).forEach(function (h) { var s = StatsDB.data.by_hour[h] || 0; if (s > sec) { sec = s; best = parseInt(h, 10); } });
            return best < 0 ? null : (best < 10 ? '0' : '') + best + ':00';
        }
    };
    ...
    var Settings = {
        ...
        setup: function () {
            ...
            try {
                ...
                Lampa.SettingsApi.addParam({ component: 'interface', param: { name: CONFIG.collect_storage, type: 'trigger', default: true }, field: { name: LANG.collect_setting } });
                Lampa.SettingsApi.addParam({ component: 'interface', param: { name: CONFIG.menu_storage, type: 'trigger', default: true }, field: { name: LANG.menu_setting }, onChange: function () { setTimeout(function () { Menu.update(); }, 100); } });
            } catch (e) {}
        },
        collecting: function () { var v = Lampa.Storage.get(CONFIG.collect_storage, true); return v !== false && v !== 'false' && v !== 0 && v !== '0'; },
        menuVisible: function () { var v = Lampa.Storage.get(CONFIG.menu_storage, true); return v !== false && v !== 'false' && v !== 0 && v !== '0'; }
    };
    ...
    var Current = {
        ...
        getVideoState: function () {
            try {
                ...
                var time = Number(video.currentTime), duration = Number(video.duration);
    ...
    /* ===================== Tracker ===================== */
    var Tracker = {
        timer: null, initialized: false, currentMovie: null, lastTickAt: 0, sessionStartWall: 0, lastTimelineWall: 0,
        init: function () {
            ...
            try {
                if (Lampa.Player && Lampa.Player.listener) {
                    ...
                    Lampa.Player.listener.follow('destroy', function () { setTimeout(function () { self.currentMovie = null; }, 5000); });
                }
            } catch (e) {}
            try { if (Lampa.PlayerVideo && Lampa.PlayerVideo.listener) Lampa.PlayerVideo.listener.follow('timeupdate', function (e) { self.onVideo(e); }); } catch (e) {}
            try { if (Lampa.Timeline && Lampa.Timeline.listener) Lampa.Timeline.listener.follow('update', function (e) { self.onTimeline(e); }); } catch (e) {}
            try {
                Lampa.Listener.follow('full', function (e) {
                    ...
                    if (cast && cast.length) { movie = Object.assign({}, movie, { credits: { cast: cast }, persons: e.data.persons || { cast: cast } }); }
                    setTimeout(function () {
                        movie = DomMeta.mergeInto(movie);
                        Media.remember(movie);
                        MetaLoader.enrich(movie);
                        if (self.currentMovie && Media.labelOf(self.currentMovie) === Media.labelOf(movie)) { self.currentMovie = Media.lastCard; }
                    }, 400);
                });
            } catch (e) {}
        ...
        onStart: function (e) {
            ...
            MetaLoader.enrich(movie, function (rich) {
                if (rich) { Tracker.currentMovie = MetaStore.applyToMovie(CardCache.get(rich) || rich); Media.remember(rich); }
            });
        },
        apply: function (time, duration, wallDelta) {
            ...
            if (movie) { movie = CardCache.get(movie) || movie; movie = MetaStore.applyToMovie(movie); }
            var meta = Media.metaFrom(movie);
            if (Media.lastCard) {
                var alt = Media.metaFrom(Media.lastCard);
                if (alt) {
                    if ((!meta || !meta.genres || !meta.genres.length) && alt.genres && alt.genres.length) { meta = meta || {}; meta.genres = alt.genres; }
        ...
        onTimeline: function (e) {
            ...
            if (!road) return;
            var time = Number(road.time), duration = Number(road.duration);
            if (!Number.isFinite(time) || time < 0) return;
            var now = Date.now();
    ...
    function StatsComponent() {
        var root = $('<div class="stv-root"></div>');
        var html = $('<div class="stv-content"></div>');
        root.append(html);
        ...
        function renderAll(container) {
            container.empty();
            container.append('<div class="stv-title">' + LANG.page_title + '</div>');
            if (!Settings.collecting()) container.append('<div class="stv-disabled">' + LANG.disabled_text + '</div>');
            var has = StatsDB.data.seconds_watched > 0 || StatsDB.data.movies_watched > 0 || StatsDB.data.episodes_watched > 0;
            if (!has) {
                container.append(addScrollFocus($('<div class="stv-empty selector">' + LANG.empty_text + '</div>')));
                container.append(resetBtn(container));
                return;
            }
            container.append(topCards());
            container.append(actorsBlock());
        ...
        function watchedCard() {
            ...
            if (list.length) {
                list.forEach(function (item) {
                    ...
                    if (url) p.css('background-image', 'url(' + url + ')'); else { p.addClass('stv-poster-empty'); p.text(item.isEpisode ? 'S' : 'F'); }
        ...
        function actorsBlock() {
            ...
            if (!list.length) {
                ...
                list.forEach(function (a) {
                    ...
                    row.append(addScrollFocus(item));
                });
            }
        ...
        function bottomRow() {
            ...
            row.append(addScrollFocus(years));
            ...
            row.append(addScrollFocus(genres));
            return row;
        }
        ...
        function yearBars() {
            ...
            data.forEach(function (d) {
                ...
                col.append('<div class="stv-bar" style="height:' + h + '%"></div><div class="stv-bar-label">' + d.label + '</div>');
                box.append(col);
            });
            return box;
        }
        ...
        function genrePie() {
            ...
            if (total <= 0) total = 1;
            var colors = ['#3b82f6', '#ef4444', '#f59e0b', '#22c55e', '#a855f7', '#06b6d4'], stops = [], acc = 0;
            ...
            list.forEach(function (g, i) { legend.append('<div class="stv-leg-item"><span class="stv-dot" style="background:' + colors[i % colors.length] + '"></span>' + g.name + ' ' + Math.round((g.seconds / total) * 100) + '%</div>'); });
            box.append(legend);
            return box;
        }
    ...
    function ActorWorksComponent(object) {
        ...
        function render(container) {
            ...
            container.append('<div class="stv-title">' + name + '</div>');
            container.append('<div class="stv-section-title">' + LANG.actor_works + '</div>');

            var works = StatsDB.actorWorks(key);
            if (!works.length) {
                container.append(addScrollFocus($('<div class="stv-empty selector">' + LANG.no_works + '</div>')));
                return;
            }
            ...
            works.forEach(function (w) {
                ...
                row.append(poster);
                row.append(info);
                list.append(addScrollFocus(row));
            });
    ...
    var Menu = {
        ...
        init: function () {
            var self = this;
            // Убрал MutationObserver, так как он мог вызвать зависание. Надежный интервал не грузит систему.
    ...
    var CSS =
        '.stv-root{position:absolute;top:0;left:0;right:0;bottom:0;overflow-y:auto;overflow-x:hidden;scroll-behavior:smooth;-webkit-overflow-scrolling:touch;}' +
        '.stv-root::-webkit-scrollbar{display:none;}' +
        '.stv-content{padding:22px 28px 40px;box-sizing:border-box;min-height:100%;color:#fff;}' +
        ...
        '.stv-card:focus,.stv-card.focus{border-color:rgba(59,130,246,0.8);background:rgba(255,255,255,0.1);}' +
        ...
        '.stv-actor:focus,.stv-actor.focus{border-color:rgba(59,130,246,0.7);background:rgba(255,255,255,0.06);}' +
        ...
        '.stv-work:focus,.stv-work.focus{border-color:rgba(59,130,246,0.7);background:rgba(255,255,255,0.09);}' +
        ...
        '.stv-panel:focus,.stv-panel.focus{border-color:rgba(59,130,246,0.7);}' +
        ...
        '.stv-reset:focus,.stv-reset.focus{border-color:rgba(255,255,255,0.45);background:rgba(255,255,255,0.1);}';
